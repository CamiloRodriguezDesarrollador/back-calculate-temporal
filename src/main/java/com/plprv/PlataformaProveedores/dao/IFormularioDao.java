...
public interface IFormularioDao extends CrudRepository<Formulario, Long> {
    ...
    @Query("SELECT p FROM Formulario p WHERE (lower(p.forNombre) LIKE %:texto% or lower(p.forDetalle) LIKE %:texto% or lower(p.forTipo)" +
            " LIKE %:texto%) AND p.forEstado = :forEstado AND (p.idEmppal = :idEmppal) ORDER BY p.forId DESC LIMIT " +
            " :numeroElementosPorPagina OFFSET :limiteInicial")
    List<Formulario> findByForEstadoPaginaFiltro(String forEstado, String texto, Integer numeroElementosPorPagina, Integer limiteInicial , Integer idEmppal);
