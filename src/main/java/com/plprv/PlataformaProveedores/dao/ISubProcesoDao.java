...
public interface ISubProcesoDao extends CrudRepository<SubProceso, Long> {
    ...
    @Query("SELECT p FROM SubProceso p WHERE (lower(p.sprNombre) LIKE %:texto%) AND (p.sprEstado = :sprEstado)" +
            " AND (p.proId = :proId OR :proId = 0) AND (p.idEmppal = :idEmppal) ORDER BY p.sprId DESC LIMIT" +
            " :numeroElementosPorPagina OFFSET :limiteInicial")
    List<SubProceso> findBySprEstadoPaginaFiltro(String sprEstado, String texto, Integer numeroElementosPorPagina, Integer limiteInicial, Integer proId, Integer idEmppal);
